...
app.get('/', (req, res) => {
    res.send(`
        ...
        <body>
            <h1>Welcome to the File System API!</h1>
            <p>Use the endpoints to create and read files.</p>
        </body>
        </html>
    `);
});
...
app.get('/files', (req, res) => {
    fs.readdir(baseFolderPath, (err, folders) => {
        ...
        folders.forEach(folder => {
            ...
            const files = fs.readdirSync(folderPath).map(file => path.join(folder, file));
